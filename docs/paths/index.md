...
不同角色 / 不同目標的人，閱讀順序與重點不同。下面這些路徑都是經過設計、可以省下你 30% 時間的閱讀地圖。

<!-- W46：開頭加「兩問題決策卡」— 多 reviewer agent 反映「不知道自己該選哪條」
...
<div class="ddia-path-cards" role="navigation" aria-label="路徑入口">
  <a href="#path-2week" class="ddia-path-card is-recommended">
    <span class="ddia-path-card-recommended">★ 多數讀者建議從這裡</span>
    <span class="ddia-path-card-eyebrow">PATH · 01</span>
    <span class="ddia-path-card-title">兩週速成</span>
    <span class="ddia-path-card-desc">沒時間 / 不確定要不要讀完——7 章覆蓋 80%</span>
  </a>
  <a href="#path-30day" class="ddia-path-card">
    <span class="ddia-path-card-eyebrow">PATH · 02</span>
    <span class="ddia-path-card-title">30 天讀通</span>
    <span class="ddia-path-card-desc">初學者版 + 工作 1-3 年完整版兩種強度</span>
  </a>
  <a href="#path-backend" class="ddia-path-card">
    <span class="ddia-path-card-eyebrow">PATH · 03</span>
    <span class="ddia-path-card-title">後端工程師</span>
    <span class="ddia-path-card-desc">交易與一致性優先、25-30 小時</span>
  </a>
  <a href="#path-de" class="ddia-path-card">
    <span class="ddia-path-card-eyebrow">PATH · 04</span>
    <span class="ddia-path-card-title">資料工程師</span>
    <span class="ddia-path-card-desc">資料流與批/串流為主、配 OLTP↔DE 橋</span>
  </a>
  <a href="#path-architect" class="ddia-path-card">
    <span class="ddia-path-card-eyebrow">PATH · 05</span>
    <span class="ddia-path-card-title">系統架構師</span>
    <span class="ddia-path-card-desc">完整深讀 12 章、40-50 小時</span>
  </a>
  <a href="#path-interview" class="ddia-path-card">
    <span class="ddia-path-card-eyebrow">PATH · 06</span>
    <span class="ddia-path-card-title">面試準備</span>
    <span class="ddia-path-card-desc">3 週系統設計面試衝刺、30 小時</span>
  </a>
</div>

::: tip 開始之前：先做 10 秒自評
**對下列任一題目答不出來嗎？** 建議先讀 [Part 0 前置知識](/part-0/)，省下後續每章 30% 的卡關時間。括號內為白話補述、看不懂專詞也能自評。

- **為什麼 P99 延遲比平均延遲更能反映使用者體驗？**<br><em>（P99 = 最慢 1% 的請求多慢；平均把所有人混在一起、看不出慢使用者的苦）</em>
- **`SELECT ... LEFT JOIN ... GROUP BY` 這個查詢做什麼？**<br><em>（SQL 把兩張表「左拼接」後再依某欄分組計數——就是「列每個使用者下了幾筆訂單、包含 0 筆的也要」）</em>
- **行程（process）與執行緒（thread）的差別？fsync 為什麼存在？**<br><em>（行程 = 一個獨立的程式實體；執行緒 = 同程式內的並行單位；fsync = 強迫把硬碟暫存寫進實體磁碟、避免斷電遺失）</em>
- **TCP「可靠」是什麼意思？網路 partial failure 有哪些情境？**<br><em>（TCP 保證封包不丟、不亂序；partial failure = 一部分節點壞了、其他還活著——比全壞更難處理）</em>
- **兩個執行緒同時對銀行帳戶 +100 為什麼會丟錢？**<br><em>（並發競態：兩者都讀到舊餘額 1000、各自算 1100 寫回、應該是 1200 卻只剩 1100）</em>
- **B-Tree 與 LSM-Tree 的根本差異？**<br><em>（兩種主流資料庫儲存結構：B-Tree 原地改、讀快；LSM-Tree 寫到新檔再合併、寫快）</em>
...
## 兩週速成：先把這條當預設 {#path-2week .role-h2 .icon-bolt}
...
## 30 天讀完 DDIA（兩種強度） {#path-30day}

想用一個月密集吃透 DDIA？依你的背景與目標選一條：

| 路徑 | 對象 | 強度 | 章節覆蓋 | ADR 練習 |
|---|---|---|---|---|
| → [**初學者 30 天暑假計畫**](/paths/30-day-beginner) | 高中 / 大二 / bootcamp 新手 | 每天 1-2 小時 | FRS「最小可用版」、可跳節先放生 | Day 30 寫 1 份 |
| → [**30 天讀通完整版**](/paths/30-day-summer-plan) | 大三大四 / 工作 1-3 年 / 升中後端 | **每天 2-3 小時** | **完整 12 章、含可跳節** | **每週 1 份共 4 份** |

兩條都配對應的 `<FirstReadShortcut>` 與 `<PartCheckpoint>` 退場 / 自評元件。
...
<div class="ddia-path-divider">
  <span class="ddia-path-divider-mark">◆ ◆ ◆</span>
  <span class="ddia-path-divider-text">以下為「依角色」進階 / 補充路徑</span>
  <span class="ddia-path-divider-hint">如果上方兩種已滿足你的需求、可跳過此區段</span>
</div>

## 後端工程師：聚焦交易與一致性 {#path-backend .role-h2 .icon-code}
...
## 資料工程師：聚焦資料流與批/串流 {#path-de .role-h2 .icon-analytics}
...
## 系統架構師：完整深讀 {#path-architect .role-h2 .icon-account_tree}
...
## 面試準備：3 週系統設計面試衝刺 {#path-interview .role-h2 .icon-rocket_launch}
...
讀完 DDIA 別停在「讀過了」、用下面 4 個工具把知識**落到可追溯的產出**：

| 工具 | 用途 | 何時用 |
|---|---|---|
| [**ADR 4-Q 模板**](/paths/adr-template) | 把 trade-off 寫成決策文件（含 7 個範例：SQS/PG/Saga/monolith/replica/Superseded/失敗 ADR）| 讀完一章後選一個你工作 / 學校真實決策、寫一份 |
| [**容量規劃工作表**](/paths/capacity-planning) | 給定 QPS + 資料量 + SLA、回推 shard / replica / 連線池 / Kafka broker / Redis 大小 | 規劃新系統 / 評估現有系統時用、3 個月重做一次 |
| [**真實事故 × DDIA 對照**](/paths/incident-postmortems) | 8 個業界 postmortem（S3 / Cloudflare / Fastly / GitHub / GitLab 等）+ 本土 blameless 範本 | 想理解「壞掉長什麼樣」+ 練習寫 5-Whys postmortem |
| [Ch9 §9.8 學界證明骨架](/part-2/ch09-consistency) | CAP/PACELC、FLP/DLS、CRDT SEC theorem 三條進階入口 | 想從工程師升級到看得懂分散式論文時 |

**完整 30 天讀通版讀者**：4 個工具按週對應到 Week 1-4 的 ADR 練習收尾、見 [30 天讀通計畫](/paths/30-day-summer-plan)。
