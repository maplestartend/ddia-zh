...
::: info W47：追問梯度 L1 / L2 / L3 慣例（取代 mock dialog）
每題「面試官追問」清單按難度分層：
- **[L1]** 應屆 / 0-1y 該答得出來（術語認得 + 三句話解釋）
- **[L2]** mid（2-4y）該答得出來（會講 trade-off、能畫圖）
- **[L3]** senior（5y+）加分（提 vendor 細節、quantify、引論文）

下方主要題目（News Feed / 轉帳 / 分散式 KV / 對帳）已標分層；其餘題目追問順序大致 L1→L3、自行對映。**面試講不出 L1 → 該題回去重讀；卡在 L2 → 可上；談得到 L3 → 加分**。
:::
...
**面試官追問**（依 L1 / L2 / L3 分層）：
- **[L1]** 「你怎麼處理超級大 V 發文時的 fan-out 爆炸？」 → 混合策略：粉絲 > 1M 改 read-time pull
- **[L2]** 「P99 = 2s、P50 = 100ms，怎麼解？」 → Ch1 「頭尾延遲放大」段、看下游 fan-out 數
- **[L3]** 「Timeline cache 跨 region 怎麼同步？」 → Ch5 multi-leader + LWW、Ch9 CAP 取捨、能講 Spanner TrueTime / DynamoDB Global Tables 細節加分
...
**面試官追問**（依 L1 / L2 / L3 分層）：
- **[L1]** 「兩個請求同時搶最後一張優惠券、你怎麼擋？」 → 三招：原子 UPDATE / 悲觀鎖 / 樂觀鎖（CAS via version）
- **[L2]** 「微服務 A 扣款、微服務 B 加庫存，A 成功 B 失敗怎麼辦？」 → Saga 補償 / TCC / Outbox + CDC（**不要選 2PC**——跨 process boundary recovery 很痛）
- **[L3]** 「PostgreSQL 跟 MySQL 預設隔離級別誰防 lost update？」 → **都不防**（READ COMMITTED）。要升 REPEATABLE READ + retry on `40001`（**只有 PG 偵測、MySQL InnoDB RR 仍不偵測**）
...
**面試官追問**（依 L1 / L2 / L3 分層）：
- **[L1]** 「為什麼 W+R>N 不保證讀到最新值？」 → Ch9 §9.1 quorum 反例三步驟（W 寫到 2/3 但 ACK 未回 client → B 讀 → C 讀到舊副本）
- **[L2]** 「Raft 的 term 跟 Paxos 的 ballot number 一樣嗎？」 → 概念相同（單調遞增、leader 標記）、細節差異 Raft 強 leader、Paxos 任意 proposer
- **[L3]** 「Spanner 的 TrueTime 是什麼？commit-wait 為何能達成 external consistency？」 → Ch9 §9.6 完整段（含 ε 不是常數、commit-wait ≈ 2ε、能講 Atomic clock + GPS 雙來源加分）
...
**面試官追問**（依 L1 / L2 / L3 分層）：
- **[L1]** 「使用者點兩次「付款」按鈕怎麼擋？」 → idempotency key（UUID + `INSERT ON CONFLICT DO NOTHING`）
- **[L2]** 「銀行 API 回傳 timeout、不知道扣款成功沒、怎麼辦？」 → 三選一：(a) 查詢 API 對帳、(b) 等下次對帳批次、(c) 設 idempotency key 安全重送
- **[L3]** 「Saga 補償失敗（refund 也失敗）怎麼辦？」 → DLQ + 人工介入、Ch9 §9.4 補償語意有強弱之分、Stripe `idempotency_key` header 24h TTL 機制加分
