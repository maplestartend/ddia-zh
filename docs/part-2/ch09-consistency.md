...
讀完 Ch5–9，你應該能用 **Part II 的詞** 回答下列分散式設計題。**答錯 ≥ 3 題建議回頭精讀對應章節**。
...
**Part II 結束自評：能不能用 Ch5-9 的詞回答這些設計題？**

1. **Read replica 為什麼會「讀到舊資料」？** read-your-writes、monotonic reads、causal reads 三種一致性分別解什麼問題？
   - *提示*：Ch5 §5.3 + §5.4

2. **Quorum W+R>N 真的保證讀到最新值嗎？** Sloppy quorum 與「並發寫」兩種情況下會怎麼壞？
   - *提示*：Ch5 §5.5 + Ch9 §9.1 ABD 反例

3. **PG REPEATABLE READ 與 MySQL InnoDB REPEATABLE READ 不一樣**——差在哪？哪個會擋 lost update？哪個不會？
   - *提示*：Ch7 §7.2 命名地獄 warning

4. **Lost update / Write skew / Phantom 三種異常**：每個各舉一個業務場景、各對應哪個隔離級別才擋得住？
   - *提示*：Ch7 §7.2 + §7.3

5. **Strict 2PL 為什麼不會 cascade abort？** SSI 為什麼能在 SI 效能下還達到可序列化？
   - *提示*：Ch7 §7.3

6. **分散式系統有哪三種「不可靠」？** Partial failure / unreliable network / unreliable clock 各自怎麼影響應用設計？
   - *提示*：Ch8 §8.1-8.3

7. **Fencing token 在解什麼問題？** 為什麼光靠 timeout-based lock 不夠？
   - *提示*：Ch8 §8.5

8. **Linearizability 與 Serializability 差別是什麼？** Strict Serializability 又是什麼？
   - *提示*：Ch9 §9.1 + 詞彙表

9. **CAP 定理常見的誤解是什麼？** 「P」實際意義是？正常時應該選 CA 嗎？
   - *提示*：Ch9 §9.2

10. **共識（Consensus）、全序廣播、線性一致 KV 三者為什麼等價？** 任挑兩個說明如何互相歸約。
    - *提示*：Ch9 §9.3 + §9.5

**答錯 ≥ 4 題**：建議從第一題對應的章節回頭重讀
**全部能答**：恭喜你跨越分散式系統最硬的一段——Part III 衍生資料、相對輕鬆
