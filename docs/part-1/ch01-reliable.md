...
<TLDR :points='[
  ...
  "<strong>故障 ≠ 失效</strong>：Fault（單元故障）必然發生，目標是讓系統整體不失效（Failure）。容錯的核心是「主動製造故障」並驗證恢復（Chaos Engineering）。",
  ...
  "<strong>P999 為何重要</strong>：Amazon 觀察到**消費量最大的客戶通常落在尾端 P999**（資料量大 → 查詢/聚合更耗時 → 延遲分布偏右）—— 也就是說「**整體 P999 不是隨機慢請求、是高價值客戶的常態**」，因此 Amazon 把 P999 列為內部 SLO（Service Level Objective）。",
...
::: tip 實務節奏：先推 scale up 到天花板、再考慮 scale out
書本氣質常給人「**分散式 = 進步**」的印象、但 SRE 業界普遍共識相反：

- **單機現代硬體很強**：AWS r6i.32xlarge 有 **128 vCPU / 1 TB RAM**、PG / MySQL 在這上面能撐單機 50K+ QPS 寫入
- **過早 shard 是 anti-pattern**：一旦分片就要面對跨片交易、re-sharding、維運複雜度，這些是「**永久成本**」、不是「下次再說的問題」
- **推到 80% 才開始 scale out**：CPU / IO / 記憶體任何一項撞 80% 持續 1 週以上、才是值得考慮 scale out 的訊號

**Stack Overflow 著名案例**：5 億 monthly PV 用 9 台 web server + 4 台 SQL Server（2024 仍是這配置）——比多數新創公司「先上 K8s」的架構簡單 10 倍、效能好 5 倍。
