...
<TLDR :points='[
  ...
  "<strong>故障 ≠ 失效</strong>：Fault（單元故障）必然發生，目標是讓系統作為整體不失效（Failure）。容錯系統的核心是「主動製造故障」並驗證恢復能力（Chaos Engineering）。",
  "<strong>負載必須用具體參數描述</strong>：Twitter 的「家庭時間軸」例子顯示，相同 QPS 在不同負載模型下會有截然不同的擴展瓶頸。",
  "<strong>延遲要用 percentile，不要用平均值</strong>：P50、P95、P99、P999 才能反映尾端使用者體驗。Amazon 觀察到延遲最差的那群請求，往往來自帳號資料最多、消費也最大的高價值客戶 —— 因此把 P999 列為內部 <strong>SLO（Service Level Objective，內部品質目標）</strong>，不容許這些人體驗惡化。",
  "<strong>可維護性的三要素</strong>：Operability（維運友善）、Simplicity（控制複雜度）、Evolvability（易於演進）。"
]' />
...
| **P999** | 0.1% 使用者慢於此值 | Amazon 的 SLO；觀察到延遲最差的請求通常來自資料量、消費量最大的客戶 |
...
也就是 <G term="stateless">無狀態服務（stateless）</G>容易 <G term="scale-out">scale out</G>；<G term="stateful">有狀態的資料系統</G>就難很多 —— 因此 DDIA 後續花了大量篇幅討論分散式資料系統的權衡。
