...
但代價：**演進規則靠約定、沒有編譯期型別檢查、跨團隊容易踩相容性坑**。要走「Kafka + 大組織」就回頭選 Protobuf / Avro + [Schema Registry](/glossary/#schema-registry)。
:::
...
訊息傳遞的世界觀（actor 內、訊息隊列外）會延續到 **Ch11 streams**——event log + stateful operator 本質上就是「**長壽 + 持久化的 actor**」。
