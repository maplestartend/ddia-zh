...
- **錯誤切法**：按技術層次切（「user service / auth service / db service」）——這只是把 monolith 內部切片再加上 RPC 開銷
- **正確切法**：按 **bounded context** 切（DDD 概念）——一個服務負責一個業務能力（「訂單」「庫存」「物流」），擁有自己的 DB schema、不跨服務 JOIN
- **粒度 trade-off**：太粗 = monolith 變形；太細 = 分散式單體（distributed monolith，所有服務都得跟 X service 同時改）
- **2024 回潮：modular monolith**——[Service Weaver](https://serviceweaver.dev/)（Google 2023）、[Encore](https://encore.dev/) 提倡「**程式碼層次的模組化、部署時還是一個 binary**」、避免微服務 overhead 但保留拆分潛力
...
**為什麼這比「重寫」好**：重寫期間業務還在跑、需求還在變、舊系統 bug fix 必須兩邊都改——絞殺榕讓「**新 + 舊 並存運行**」變成預設狀態、不是過渡。
...
**Part I 結束自評：能不能用 Ch1-4 的詞回答這些設計題？**
