---
title: 0.4 資料結構地基
description: Hash table、B-Tree、外部排序、Big-O —— 接 0.3 SQL 索引、Ch3 直接會用到
---
...
<TLDR :points='[
  "<strong>Big-O 給上界、不給常數</strong>：O(log n) 通常比 O(n) 好，但 n 小時常數可以反轉勝負 —— 為什麼小資料量用陣列線性搜尋反而比 hash table 快。",
  "<strong>Hash table</strong>：平均 O(1) 存取，靠雜湊函式把 key 映射到桶。記憶體 KV 的基礎 —— Redis / Memcached、磁碟上的 Bitcask。",
  "<strong>B-Tree</strong>：n 路平衡樹、每節點是磁碟頁大小。寫入要原地更新（需 WAL 保 crash recovery）、讀快。傳統關聯式 DB 的索引結構。",
  "<strong>外部排序（external sort）</strong>：資料大於 RAM 時、分段排序 + 合併。MapReduce 的 shuffle、SSTable 的合併、Spark sort-merge join 全用這個。",
  "<strong>Ch3 在比較 B-Tree vs LSM-Tree 兩家儲存引擎</strong> —— 沒有這章的基礎會卡。"
]' />
...
<Quiz chapter-id="p0-ds" :questions='[
  ...
  },
  {
    ...
    "options": [
      "B-Tree 比較老、LSM 比較新",
      "B-Tree 原地更新（每次寫入直接改頁面、屬隨機 I/O）；LSM 只追加寫到記憶體 + 背景合併（屬順序 I/O）—— 兩家在「讀寫吞吐量、寫放大、磁碟模式」三軸上各有取捨",
      "B-Tree 用於 OLTP、LSM 用於 OLAP",
      "B-Tree 寫入快、LSM 讀取快"
    ],
    "answer": 1,
    "explanation": "本章只到「LSM 用追加寫 + 合併」這層直覺、Ch3 整章才會展開兩家在讀放大 / 寫放大 / 空間放大三個維度的詳細權衡。一句話記：LSM 真正的優勢是「順序寫」（B-Tree 是隨機原地更新）—— 在 HDD 與早期 SSD 都是巨大優勢。寫密集（Cassandra、RocksDB）選 LSM；讀密集且 ACID 選 B-Tree。"
  }
]' />

<NextChapterBridge next-link="/part-0/os" next-title="0.5 作業系統地基">
本章 §3 提到 B-Tree 原地更新需要 WAL —— 但 WAL 為什麼又快又安全？答案在下一章：OS 的 page cache 讓「先寫一筆順序的 log」遠快於「原地改隨機頁面」、fsync 又保證 commit 不會在斷電時消失。Ch3 / Ch7 / Ch8 都假設你看得懂這條鏈。
</NextChapterBridge>
