...
<TLDR :points='[
  "<strong>資料密集 ≠ 計算密集</strong>：瓶頸在資料量、資料複雜度、資料變化速度，不在 CPU。",
  "<strong>後端服務多半設計成 stateless</strong>：應用層不存請求間狀態、全交給 DB / 快取 / 訊息隊列 —— 加機器水平擴展才容易。",
  "<strong>真正難的是 stateful 元件</strong>：DB、Kafka、Redis cluster 才是 DDIA 的核心戰場。",
  "<strong>典型現代後端 = 五種元件的拼裝</strong>：OLTP DB、OLAP / Search、Cache、Message Queue、Stream Processor。會選、會接、會權衡 —— 就是 Ch1 想教的能力。"
]' />
...
```
Client → API Gateway / LB（Load Balancer，負載平衡器）→ 應用服務（多台）
                                  │
                ┌─────────────────┼─────────────────┐
                ▼                 ▼                 ▼
...
- **OLAP（Online Analytical Processing）**：少量大查詢（這個月誰買最多）—— 資料分析師的 BI 工具背後
