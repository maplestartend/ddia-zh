...
::: warning TCP Keepalive 預設 7200 秒太長（SRE 必調）
Linux 預設 `tcp_keepalive_time = 7200`（2 小時）才送第一個 keepalive probe——這對任何「需要在 1 分鐘內知道對方掛了」的應用都太長。

**典型問題場景**：
1. 你的應用持有 DB 連線、DB 端網路設備（防火牆 / LB）默默把 TCP session 從 NAT table 移除
2. 應用以為連線還活、繼續送 query、實際送進黑洞
3. 你的連線池逐漸耗盡（每條連線都「看起來活、實際死」）
4. 90 分鐘後業務開始「莫名其妙超時」、但 monitoring 看不出原因
...
# 總計：60s 閒置 + 60s probe = 2 分鐘內偵測到失連
```

**應用層補強**：
- **連線池配 healthCheck**（HikariCP、pgbouncer）：每 30s 拿一條連線跑 `SELECT 1`、失敗就丟掉
- **設 connection_timeout / socket_timeout** 在 client 端、不要永遠等
