...
### Saga {#saga}
**中文**：Saga 模式
跨多服務 / 多 DB 的長交易拆成一系列**本地交易 + 補償交易**，避免用 2PC。每個本地交易成功後對外發布事件、下一步接著做；任一步失敗就依序執行先前步驟的**補償**（如「扣款」對應「退款」）。
- ✓ 適合 microservices、跨異質系統、長執行時間
- ✗ 失去 ACID isolation——中間狀態對外可見、需業務層處理[冪等](#idempotent)
- 兩種協調風格：**Orchestration**（中央 orchestrator 發指令）vs **Choreography**（各服務監聽事件自主決策）

DDIA 主文未直接展開、但與 Ch11 stream processing + Ch12 Unbundling the database 是同源思想——「**事件 log 當骨幹、補償取代 rollback**」。常見實作：AWS Step Functions、Temporal、Camunda。
