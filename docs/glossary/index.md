...
- **跨列版本**（A3 / 經典 write skew）：例「至少要有一位醫生值班」—— T1 T2 各讀到 2 人值班、各改自己的 active=false → 違反 ≥ 1 約束
- **基於不存在性的版本**（A5B / phantom-based write skew）：例「同一時段不能撞會議」—— T1 T2 各讀「該時段沒會議」、各插一個 → 兩個會議撞期
