...
W49 已完成 components.css（原 2772 行）拆分為 6 個子檔，現在的結構：
...
│   ├── disclaimer-scenario.css   (135 行) Hero disclaimer / Scenario badge / Hero CTA secondary nav
│   ├── dashboard.css             (306 行) Progress / Dashboard / Stat card / 錯題本
│   ├── chapter-cards.css         (657 行) Chapter cards / Part header / Hero / custom-block / Card status
│   ├── vp-overrides.css          (567 行) VP nav/sidebar/outline/search/footer + Mermaid 暗色 + Dashboard resume/ceremony + reading-progress
│   └── diagrams-paths.css        (654 行) figures / DecisionTree / SequenceFlow / path-decision / em-mark / glossary-h2 / recent-update
