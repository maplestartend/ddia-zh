...
`custom.css` 目前 ~800 行（含 disclaimer / scenario badge / dark CTA / sticky elevated）。未來再長就拆成 `tokens.css`、`base.css`、`components.css`、`layout.css`，在 `theme/index.ts` 多檔 import。**注意**：曾嘗試 PowerShell 切檔導致中文註解 mojibake，**必須用 Node script + utf-8 encoding** 切分。
