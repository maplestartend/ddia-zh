...
## W52 視覺 / A11y / 內容 audit 修正輪（10 agent 共識 P0+P1+P2 全執行）

10 agent（5 visual reviewer × 5 senior cross-review）對排版 / 互動 / 響應式 / 階層 / 內容 IA 五維度 audit、產 14 條候選清單、senior 交叉審後同意做 10 條、明確拒絕 4 條（純品味）。

**P0 必修（5 條、reviewer 共識 + WCAG 違規）**：
- **FirstReadShortcut raw render fix**：`<FirstReadShortcut>` 在 30-day-beginner.md 內 markdown link 渲染成 raw code、破壞學習引導；改用「第一次讀建議路徑」純文字（5 處 .md 修正）
- **ChapterMeta tag mobile 黏連 fix**（WCAG 2.5.5 + iOS HIG 44pt fail）：editorial-marks.css `@media (max-width: 480px)` 加 `gap` + `min-height: 32px` + tag-link `cursor: pointer`、`?` icon opacity 0.55→0.85
- **Quiz CLS guard**（WCAG 2.4.11 Focus Not Obscured）：[Quiz.vue](docs/.vitepress/theme/components/Quiz.vue) submit() 後 `nextTick(scrollTo Quiz header)` 把讀者眼線錨回 Q1 + `contain: layout`
- **v380 表格 reflow**（WCAG 1.4.10 critical）：base.css `@media (max-width: 480px) .vp-doc table { display: block; overflow-x: auto; }` — Ch5 三家落差表第一欄不再被砍
- **GlossaryTerm 視覺差異化**（WCAG 1.4.1 Use of Color）：dotted underline 1px → 2px（守 Visual #3 反對「換深色」、用加寬而非換色區分 link vs tooltip）

**P1（5 條）**：
- 30 天計畫週標頭 `｜` 強制斷行 → base.css h2 加 `word-break: keep-all; overflow-wrap: break-word`
- Chapter card 可點性弱補強 → `cursor: pointer` + `::before` opacity 0→0.25 baseline 預示（Visual #3 守則：**不加 lift shadow / 不換 brand color**）
- ChapterMeta tag-link hover 加強 → hover 時 `border-bottom-width: 2px`（FE #2 部分採納）
- Floating progress chip 1024 顯示驗證 → @media 從 `max-width: 1024` 改 `max-width: 1023`（CLAUDE.md 承諾 ≥1024 才顯示、原條件含等號漏 1024 桌面）
- CH 編號 eyebrow 與標題距離 → chapter-card-head margin-bottom 6→10px（Visual #3 漏項）

**P2（0 條，原 3 條皆已實現或不需要）**：
- Details 預設行為一致 — ChapterNote 已是 `:open="hasContent"`（內容才展、與 InterviewBlock 同步、reviewer 觀察誤判）
- Floating chip ARIA live — template 已有 `role="status" aria-live="polite"`（reviewer 漏看）
- CH 編號距離 — 升 P1 已處理

**明確拒絕（10 條）**：
- Hero 6 種間距收 token（Visual #3「節奏在於不齊」）
- Chapter card padding 反模式 3:1（FE「連體網格刻意」）
- 章末區塊邊界混亂（Visual + FE：W44 audit 已過、章末沈澱必要）
- 1024 三欄行寬 18 字/行（FE「高風險、minmax 改會破連體網格」）
- Progress button hover 加強（Visual：書頁是平的、不該 lift）
- Page-note disclaimer 上下 100px（FE：reviewer 看錯、實際 32px）
- 章末 5 元件分層（IA polish、會撒 dinkus 變更密）
- OLTP-DE 跳回 TOC（IA polish）
- ★ 浮動入口（glossary 已有 GlossaryStarLinks）
- 30 天三色情緒色（Visual：破 mahogany + accent 兩色原則）

**驗收全綠**：type-check / build 5.10s / 24 tests / 6 lint。
