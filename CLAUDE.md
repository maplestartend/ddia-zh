...
- 工具腳本：[scripts/](scripts/) — lint-glossary / lint-tldr / screenshot
- 公開資源：[LICENSE](LICENSE) / [NOTICE.md](NOTICE.md) / [.github/ISSUE_TEMPLATE/](.github/ISSUE_TEMPLATE/) / [.github/workflows/deploy.yml](.github/workflows/deploy.yml)
...
`custom.css` 已超過此上限（以 `wc -l` 為準，含 disclaimer / scenario badge / dark CTA / sticky elevated）。拆成 `tokens.css`、`base.css`、`components.css`、`layout.css`，在 `theme/index.ts` 多檔 import。**注意**：曾嘗試 PowerShell 切檔導致中文註解 mojibake，**必須用 Node script + utf-8 encoding** 切分。**禁止用 PowerShell 處理 `docs/.vitepress` 之類含 `.` 開頭的子目錄路徑**——曾因路徑分隔誤判產生 8 個 mojibake 空目錄污染 repo 根。
